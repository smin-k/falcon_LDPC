...
// 	for i := 0; i < 1000; i++ {
// 		fmt.Printf("----------------------------------node %d-------------------------------------------\n", i)
// 		// Generate proof and random value with each node's key
// 		pk, sk, _ := ed25519.GenerateKey(nil)
// 		rNumber := DoTestECVRF(pk, sk, msg[:], true)

// 		// The node whose first character of random value is "a" is selected
// 		res := sortition(rNumber)
// 		fmt.Printf("Sortition: %t\n", res)
// 		if res {
// 			winVrf = append(winVrf, i)
// 		}
// 		fmt.Println("-----------------------------------------------------------------------------------")
// 	}

// 	fmt.Printf("Selected nodes: %v,\ntotal %d nodes are selected.\n", winVrf, len(winVrf))
// 	fmt.Println("Sifted nodes now run ECCPoW")
// 	runEccpow(winVrf)
// }
